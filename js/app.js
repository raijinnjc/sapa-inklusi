...
function showToast(message, type = 'info') {
    ...
    toast.style.borderColor = type === 'error' ? '#FF453A' : '#0066cc';
    
    toast.innerHTML = `
        <span class="material-symbols-outlined text-base ${type === 'error' ? 'text-[#FF453A]' : 'text-[#0066cc]'}">
...
function stopVoice() {
    ...
    if (btn) btn.classList.remove('bg-[#0066cc]');
...
function sendAiMessage(promptText) {
    ...
    userDiv.innerHTML = `
        <div class="bg-[#0066cc] text-white p-4 rounded-2xl rounded-tr-sm text-xs leading-relaxed">
    ...
    aiDiv.innerHTML = `
        <div class="w-7 h-7 rounded-full bg-[#0066cc] text-white font-bold text-[10px] flex items-center justify-center shrink-0">AI</div>
    ...
    list.forEach((p, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td class="py-3.5 px-4">
                <span class="inline-flex items-center gap-1 typo-fine-print font-semibold px-2 py-0.5 rounded-full ${isCert ? 'bg-[#0066cc]/10 text-[#0066cc]' : 'bg-[#e0e0e0] text-[#7a7a7a]'}">
                    ${isCert ? 'Bersertifikat' : 'Dalam Progres'}
                </span>
            </td>
            <td class="py-3.5 px-4 text-[#1d1d1f]">${p.competencies[0]}</td>
            <td class="py-3.5 px-4 typo-fine-print font-semibold ${p.availability === 'TERSEDIA' ? 'text-[#0066cc]' : 'text-[#7a7a7a]'}">
        ...
        row.innerHTML = `
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="typo-fine-print px-2 py-0.5 rounded-full ${isNeed ? 'bg-[#FF453A]/10 text-[#FF453A]' : 'bg-[#0066cc]/10 text-[#0066cc]'} font-semibold">${isNeed ? 'Butuh PIB' : 'Terpenuhi'}</span>
                </div>
                <p class="typo-caption text-[#7a7a7a]">${s.region} • ${s.distanceKm} km • Siswa: ${s.studentsCount} anak</p>
            </div>
            <div class="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                <div class="text-right">
                    <span class="typo-caption-strong text-[#0066cc] block">${s.matchingScore}% Match</span>
...
function renderSessionsList() {
    ...
    list.forEach(s => {
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 px-4">
                <span class="typo-fine-print font-semibold px-2 py-0.5 rounded-full ${s.status === 'SELESAI' ? 'bg-[#0066cc]/10 text-[#0066cc]' : 'bg-[#e0e0e0] text-[#7a7a7a]'}">
                    ${s.status}
                </span>
            </td>
            <td class="py-3 px-4 text-center" onclick="event.stopPropagation()">
                ${isVerif ? `
                    <span class="typo-fine-print font-bold text-[#0066cc]">✓ Terverifikasi</span>
                ` : `
...
function switchMobileTab(tabName) {
    ...
    document.querySelectorAll('.mob-nav-btn').forEach(btn => {
        if (btn.getAttribute('data-tab') === tabName) {
            btn.className = "mob-nav-btn flex flex-col items-center text-[#0066cc] font-bold";
        } else {
